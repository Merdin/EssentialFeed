			...
			sut.retrieve { firstResult in
				sut.retrieve { secondResult in
					switch (firstResult, secondResult) {
						...
						default:
							XCTFail("Expected retrieving twice from non empty cache to deliver same foudn result with feed \(feed) and timestamp \(timestamp), got \(firstResult) and \(secondResult) instead")
					}
